...
def run(argv=None):
    ...
    offer_importer_pipeline_options = pipeline_options.view_as(OfferImporterPipelineOptions)
    ...
    | 'Parsing offers' >> beam.Map(lambda line: Offer.parseFromCsvLine(line)) \
...
class Offer(object):
    ...
    def parseFromCsvLine(line):
        ...
        reader = csv.DictReader([line], delimiter=',', fieldnames=Offer.csv_header.split(','))
        line_dict = reader.next()
